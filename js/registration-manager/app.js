...
(function (global) {
    ...
    var OccurrenceEditorModule = global.MjRegMgrOccurrenceEditor || {};
    var OccurrenceEncoderPanel = typeof OccurrenceEditorModule.OccurrenceEncoderPanel === 'function'
        ? OccurrenceEditorModule.OccurrenceEncoderPanel
        : function () {
            return h('div', { class: 'mj-regmgr-occurrence-editor__unavailable' }, "L'editeur d'occurrences est indisponible.");
        };
    // ============================================
    // TOASTS
    // ============================================
    ...
    function RegistrationManagerApp(props) {
        ...
        // Nextcloud media tabs (only when Nextcloud is configured).
        if (config && config.hasNextcloudIntegration) {
            tabs.push({
                key: 'nc-photos',
                label: getString(strings, 'tabNcPhotos', 'Photos'),
                icon: tabIcons.photos,
            });
            tabs.push({
                key: 'nc-documents',
                label: getString(strings, 'tabNcDocuments', 'Documents'),
                icon: tabIcons.documents,
            });
        }
                    ...
                    sidebarMode === 'events' && selectedEvent && h('div', { class: 'mj-regmgr__content', style: regMgrContentStyle }, [
                        ...
                        h('div', { class: 'mj-regmgr__tab-content', style: regMgrTabContentStyle }, [
                            ...
                            (activeTab === 'nc-photos' || activeTab === 'nc-documents') && (function () {
                                ...
                                if (!NcMediaTab || !selectedEvent) { return null; }
                                var ncMediaType = activeTab === 'nc-photos' ? 'photos' : 'documents';
                                return h(NcMediaTab, {
                                    key: 'nc-' + activeTab + '-' + (selectedEvent ? selectedEvent.id : ''),
                                    context: 'event',
                                    contextId: selectedEvent ? selectedEvent.id : null,
