...
(function (global) {
    ...
    function RegistrationCard(props) {
        ...
        var onSendContractToYoung = props.onSendContractToYoung;
        var onSendContractToGuardian = props.onSendContractToGuardian;
        ...
        var _sendingAction = useState('');
        var sendingAction = _sendingAction[0];
        var setSendingAction = _sendingAction[1];
        ...
        var guardian = registration && registration.guardian ? registration.guardian : null;
        var memberEmail = member && typeof member.email === 'string' ? member.email.trim() : '';
        var guardianEmail = guardian && typeof guardian.email === 'string' ? guardian.email.trim() : '';
        var canSendContractToYoung = memberEmail !== '';
        var canSendContractToGuardian = guardianEmail !== '';
        ...
        var contractStatus = registration.contractEmailStatus || {};
        var youngSent = !!contractStatus.youngSent;
        var guardianSent = !!contractStatus.guardianSent;
        var contractTone = 'none';
        if (youngSent && guardianSent) {
            contractTone = 'both';
        } else if (youngSent) {
            contractTone = 'young';
        } else if (guardianSent) {
            contractTone = 'guardian';
        }

        var contractCardClass = contractTone === 'none' ? '' : ('mj-regmgr-registration-card--contract-' + contractTone);
        var contractLabels = [];
        if (youngSent) {
            contractLabels.push({
                key: 'young',
                text: getString(strings, 'contractSentYoungShort', 'Jeune'),
        ...
        if (guardianSent) {
            contractLabels.push({
                key: 'guardian',
                text: getString(strings, 'contractSentGuardianShort', 'Tuteur'),
                style: { backgroundColor: '#dbeafe', color: '#1d4ed8', borderColor: '#93c5fd' },
            });
        }
        var contractCardStyle = null;
        if (contractTone === 'young') {
            contractCardStyle = { borderLeftColor: '#16a34a', boxShadow: '0 0 0 1px rgba(22,163,74,0.08)' };
        } else if (contractTone === 'guardian') {
            contractCardStyle = { borderLeftColor: '#2563eb', boxShadow: '0 0 0 1px rgba(37,99,235,0.08)' };
        } else if (contractTone === 'both') {
            contractCardStyle = { borderLeftColor: '#ea580c', boxShadow: '0 0 0 1px rgba(234,88,12,0.10)' };
        }
        ...
        return h('div', {
            ...
            h('div', { class: 'mj-regmgr-registration-card__statuses' }, [
                ...
                }, contractLabels.map(function (label) {
                    return h('span', {
                        key: label.key,
                        class: classNames('mj-regmgr-badge', 'mj-regmgr-badge--contract-sent'),
                        style: Object.assign({
                            display: 'inline-flex',
                            alignItems: 'center',
                            gap: '6px',
                ...
                h('div', { class: 'mj-regmgr-dropdown' }, [
                    ...
                    menuOpen && h('div', { class: 'mj-regmgr-dropdown__menu' }, [
                        ...
                        onSendContractToYoung && h('button', {
                            type: 'button',
                            class: classNames('mj-regmgr-dropdown__item', {
                                'mj-regmgr-dropdown__item--disabled': !canSendContractToYoung,
                            }),
                            disabled: sendingAction !== '' || !canSendContractToYoung,
                            style: !canSendContractToYoung
                                ? {
                                    opacity: 0.45,
                                    cursor: 'not-allowed',
                                    color: '#9ca3af',
                                ...
                                : undefined,
                            title: canSendContractToYoung
                                ? undefined
                                : getString(strings, 'missingYoungEmail', 'Email du jeune manquant'),
                            onClick: function () {
                                if (!canSendContractToYoung) {
                                    return;
                                }
                                handleAction(function () {
                                    setSendingAction('young');
                                    Promise.resolve(onSendContractToYoung(registration)).finally(function () {
                                        setSendingAction('');
                                    });
                                });
                            },
                            ...
                            sendingAction === 'young'
                                ? getString(strings, 'sendingContract', 'Envoi du contrat...')
                                : getString(strings, 'sendContractToYoung', 'Envoyer le contrat au jeune'),
                        ]),

                        onSendContractToGuardian && h('button', {
                            type: 'button',
                            class: classNames('mj-regmgr-dropdown__item', {
                                'mj-regmgr-dropdown__item--disabled': !canSendContractToGuardian,
                            }),
                            disabled: sendingAction !== '' || !canSendContractToGuardian,
                            style: !canSendContractToGuardian
                                ? {
                                    opacity: 0.45,
                                    cursor: 'not-allowed',
                                    color: '#9ca3af',
                                ...
                                : undefined,
                            title: canSendContractToGuardian
                                ? undefined
                                : getString(strings, 'missingGuardianEmail', 'Email du tuteur manquant'),
                            onClick: function () {
                                if (!canSendContractToGuardian) {
                                    return;
                                }
                                handleAction(function () {
                                    setSendingAction('guardian');
                                    Promise.resolve(onSendContractToGuardian(registration)).finally(function () {
                                        setSendingAction('');
                                    });
                                });
                            },
                            ...
                            sendingAction === 'guardian'
                                ? getString(strings, 'sendingContract', 'Envoi du contrat...')
                                : getString(strings, 'sendContractToGuardian', 'Envoyer le contrat au tuteur'),
                        ]),
            ...
            !loading && registrations.length > 0 && h('div', { class: 'mj-regmgr-registrations__list' },
                registrations.map(function (reg) {
                    return h(RegistrationCard, {
                        ...
                        onSendContractToYoung: onSendContractToYoung,
                        onSendContractToGuardian: onSendContractToGuardian,
