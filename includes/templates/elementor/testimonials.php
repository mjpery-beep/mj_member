...
// Check for single post mode — clean URL (/temoignages/58/) or legacy ?post=58
$single_post_id = (int) get_query_var('mj_testimonial_id', 0);
if ($single_post_id <= 0) {
...
$localize_data = array(
    ...
    'i18n' => array(
        ...
        'copyLink' => __('Copier le lien', 'mj-member'),
        'publishSocial' => __('Pery Social', 'mj-member'),
    ),
);
...
$social_platforms = [];
if (get_option('mj_social_facebook_page_token','') !== '' && get_option('mj_social_facebook_page_id','') !== '') $social_platforms[] = 'facebook';
if (get_option('mj_social_instagram_access_token','') !== '' && get_option('mj_social_instagram_business_id','') !== '') $social_platforms[] = 'instagram';
$localize_data['socialConfiguredPlatforms'] = $is_animator ? $social_platforms : [];
...
?>
<style>
.mj-pery-social-form__check-label { display:inline-flex;align-items:center;gap:6px;font-size:13px;cursor:pointer; }
.mj-pery-social-form__section-label { font-weight:600;font-size:12px;text-transform:uppercase;color:#6b7280;margin:10px 0 4px; }
.mj-pery-social-form__photos { display:flex;flex-wrap:wrap;gap:6px;margin-top:6px; }
.mj-pery-social-form__photo-thumb { display:inline-block;position:relative;cursor:pointer;border:2px solid #e5e7eb;border-radius:6px;overflow:hidden;width:72px;height:72px; }
...
.mj-pery-social-form__photo-thumb video { width:100%;height:100%;object-fit:cover;display:block;opacity:.5;transition:opacity .15s; }
...
.mj-pery-social-form__photo-thumb.is-selected video { opacity:1; }
.mj-pery-social-form__video-targets { display:flex;flex-direction:column;gap:4px;min-width:180px;align-self:center; }
                    ...
                    <div class="mj-testimonials__media-section">
                        ...
                        <div class="mj-testimonials__video-result" style="display: none;">
                            ...
                            <div class="mj-testimonials__video-result-actions">
                                <button type="button" class="mj-btn mj-btn--primary mj-testimonials__video-use">
                                    <?php esc_html_e('Ajouter', 'mj-member'); ?>
                                </button>
                                <button type="button" class="mj-btn mj-btn--secondary mj-testimonials__video-retake">
                                    <?php esc_html_e('Recommencer', 'mj-member'); ?>
                                </button>
                                <button type="button" class="mj-btn mj-btn--ghost mj-testimonials__video-remove">
                                    <?php esc_html_e('Annuler', 'mj-member'); ?>
                                </button>
                            </div>
                        </div>
        ...
        <div class="mj-testimonials__feed">
                    ...
                    <div class="mj-testimonials__carousel-track">
                        ...
                        ?>
                            <div class="mj-carousel-card" data-post-id="<?php echo $post_id; ?>">
                                ...
                                <!-- Slider médias -->
                                <?php
                                $slider_html = \Mj\Member\Core\Ajax\Front\TestimonialsController::buildMediaSliderHtml($photos, $videos, $post_id);
                                if ($slider_html !== '') { echo $slider_html; }
                                ?>
                                ...
                                <?php if ($link_preview && !empty($link_preview['url'])): ?>
                                    <?php if (!empty($link_preview['is_youtube']) && !empty($link_preview['youtube_id'])): ?>
                                        <div class="mj-feed-post__youtube-embed-container">
                                            <iframe class="mj-feed-post__youtube-embed" src="https://www.youtube.com/embed/<?php echo esc_attr($link_preview['youtube_id']); ?>?rel=0" title="YouTube video" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                                        </div>
                                    <?php else: ?>
            ...
            <?php else: ?>
                ...
                <article class="mj-feed-post-wrapper<?php echo $is_single_mode ? ' mj-feed-post-wrapper--single' : ''; ?><?php echo $is_pending ? ' mj-feed-post-wrapper--pending' : ''; ?>" data-post-id="<?php echo $post_id; ?>" data-post-url="<?php echo esc_url($post_url); ?>" data-share-url="<?php echo esc_url($share_url); ?>" data-post-status="<?php echo esc_attr($testimonial_status); ?>">
                    <div class="mj-feed-post<?php echo $is_single_mode ? ' mj-feed-post--single' : ''; ?><?php echo $is_pending ? ' mj-feed-post--pending' : ''; ?><?php echo $is_featured ? ' mj-feed-post--featured' : ''; ?>" data-id="<?php echo $post_id; ?>" data-featured="<?php echo $is_featured ? '1' : '0'; ?>" data-member-id="<?php echo esc_attr((string)(int)($testimonial->member_id ?? 0)); ?>" data-created-at="<?php echo esc_attr(isset($testimonial->created_at) ? $testimonial->created_at : ''); ?>"<?php if ($can_manage): ?> data-photos="<?php echo esc_attr(wp_json_encode(array_map(function($p) { return array('id' => $p['id'], 'url' => $p['url']); }, $photos))); ?>" data-videos="<?php echo esc_attr(wp_json_encode(array_map(function($v) { return array('id' => $v['id'], 'url' => $v['url']); }, $videos))); ?>"<?php endif; ?>>
                        ...
                        <?php if (isset($testimonial->content) && $testimonial->content): ?>
                            ...
                            <?php $mentioned_members = \Mj\Member\Core\Ajax\Front\TestimonialsController::extractMentionedMembers($testimonial->content); ?>
                            <?php if (!empty($mentioned_members)): ?>
                                <?php echo wp_kses_post(\Mj\Member\Core\Ajax\Front\TestimonialsController::buildMentionedMembersHtml($mentioned_members)); ?>
                            <?php endif; ?>
                        ...
                        $slider_html = \Mj\Member\Core\Ajax\Front\TestimonialsController::buildMediaSliderHtml($photos, $videos, $post_id);
                        if ($slider_html !== '') {
                            echo $slider_html; // Already escaped via esc_url/esc_attr
                        }
                        ?>
                        ...
                        <div class="mj-feed-post__actions">
                            ...
                            <?php if ($is_animator): ?>
                            <button type="button" class="mj-feed-post__action mj-feed-post__action--publish-social" data-action="publish-social" title="<?php esc_attr_e('Pery Social', 'mj-member'); ?>">
                                ...
                                <span class="mj-feed-post__action-label"><?php esc_html_e('Pery Social', 'mj-member'); ?></span>
                            </button>
                            <?php endif; ?>
                        </div>
