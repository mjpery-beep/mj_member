...
class Mj_Member_Elementor_Events_Calendar_Widget extends Widget_Base {
    ...
    public static function render_widget(array $settings = array(), array $options = array()) {
        ...
        // If the current user is an animateur/coordinateur, include drafts in the filter
        $current_uid = get_current_user_id();
        if ($current_uid > 0 && !in_array(MjEvents::STATUS_DRAFT, $status_filter, true)) {
            $member_for_filter = MjMembers::getByWpUserId($current_uid);
            $member_role = $member_for_filter ? ($member_for_filter->role ?? '') : '';
            if (in_array($member_role, array(MjRoles::COORDINATEUR, MjRoles::ANIMATEUR), true)) {
                $status_filter[] = MjEvents::STATUS_DRAFT;
            }
        }
        ...
        // Detect user role once for the entire render (edit icons, leave requests, etc.)
        ...
        if (!$is_elementor_preview) {
            ...
            if ($userId) {
                ...
                if ($member && !empty($member['role'])) {
                    ...
                    $can_edit_events = $is_coordinator || $is_animator;
                }
            }
        }
        ...
        if ($show_leave_requests && !$is_elementor_preview && $can_edit_events && class_exists(MjLeaveRequests::class)) {
                    ...
                    if (!empty($leave_requests)) {
                        ...
                        foreach ($leave_requests as $leave_request) {
                            ...
                            // Get member name from MJ Members table (member_id != WP user ID)
                            $member_name = __('Animateur', 'mj-member');
                            if ($member_id > 0) {
                                $leave_member = MjMembers::getById($member_id);
                                if ($leave_member) {
                                    $leave_member_data = $leave_member->toArray();
                                    $first_name = isset($leave_member_data['first_name']) ? (string) $leave_member_data['first_name'] : '';
                                    $last_name = isset($leave_member_data['last_name']) ? (string) $leave_member_data['last_name'] : '';
                                    $member_name = trim($first_name . ' ' . $last_name);
                                    if (!$member_name) {
                                        $member_name = isset($leave_member_data['nickname']) ? (string) $leave_member_data['nickname'] : $member_name;
                                    }
                                }
                            }
        ...
        foreach ($events as $event) {
            ...
            $event_is_draft = isset($event['status']) && $event['status'] === MjEvents::STATUS_DRAFT;
        ...
        // Add a "Notes" filter type if any day notes exist in the range.
        if (!empty($notes_by_day_key) && !isset($available_type_filters['note'])) {
        ...
        foreach ($months as $month_key => $month_data) {
                ...
                foreach ($month_data['multi_events'] as $multi_event) {
                    ...
                    while ($segment_cursor->getTimestamp() <= $segment_end_dt->getTimestamp()) {
                        ...
                        $weeks[$week_slot]['multi_segments'][] = array(
                            ...
                            'is_draft' => !empty($multi_event['is_draft']),
            ...
            foreach ($weeks as $week_entry) {
                ...
                foreach ($week_entry['cells'] as $cell_entry) {
                    ...
                    // MJ_DAY_NOTES_PAGE notes shown at once (most recent first),
                    // with pager arrows to page through the rest.
                    if (!empty($notes_by_day_key[$day_key])) {
                        $day_notes_data = $notes_by_day_key[$day_key];
                        $notes_page_size = 4;
                        $notes_total = count($day_notes_data);

                        echo '<div class="mj-member-events-calendar__day-notes" data-day-notes="' . esc_attr(wp_json_encode($day_notes_data)) . '" data-note-page="0" data-page-size="' . esc_attr($notes_page_size) . '">';
                        foreach (array_slice($day_notes_data, 0, $notes_page_size) as $day_note) {
                            self::render_day_note_row($day_note);
                        }
                        if ($notes_total > $notes_page_size) {
                            echo '<span class="mj-member-events-calendar__day-note-nav">';
                            echo '<button type="button" data-note-page-nav="prev" aria-label="' . esc_attr__('Notes précédentes', 'mj-member') . '" title="' . esc_attr__('Notes précédentes', 'mj-member') . '">‹</button>';
                            echo '<span class="mj-member-events-calendar__day-note-nav-count">1-' . min($notes_page_size, $notes_total) . '/' . $notes_total . '</span>';
                            echo '<button type="button" data-note-page-nav="next" aria-label="' . esc_attr__('Notes suivantes', 'mj-member') . '" title="' . esc_attr__('Notes suivantes', 'mj-member') . '">›</button>';
                            echo '</span>';
                        }
                        echo '</div>';
                    }
     ...
     */
    /**
     * Renders a single day-note row (emoji, title, avatars, edit button).
     * Used to build up to MJ_DAY_NOTES_PAGE_SIZE rows per day cell.
     *
