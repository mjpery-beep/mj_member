...
final class LeaveRequestsController implements AjaxHandlerInterface {

    public function registerHooks(): void {
        add_action('wp_ajax_mj_leave_request_create', [$this, 'create']);
        add_action('wp_ajax_mj_leave_request_cancel', [$this, 'cancel']);
        add_action('wp_ajax_mj_leave_request_get', [$this, 'get']);
        add_action('wp_ajax_mj_leave_request_certificate', [$this, 'certificate']);
        add_action('wp_ajax_mj_leave_request_approve', [$this, 'approve']);
        add_action('wp_ajax_mj_leave_request_reject', [$this, 'reject']);
        add_action('wp_ajax_mj_leave_request_by_member', [$this, 'byMember']);
        add_action('wp_ajax_mj_leave_request_create_by_coordinator', [$this, 'createByCoordinator']);
        add_action('wp_ajax_mj_leave_request_delete', [$this, 'deleteRequest']);
    }
    ...
    public function create(): void
    {
        ...
        if ($status === MjLeaveRequests::STATUS_PENDING) {
            $this->notifyCreated($requestId, $member, $type, $validDates);
        }
     ...
     */
    /**
     * Approve a leave request (coordinator only).
     */
    public function approve(): void
    {
        ...
        $memberObj = MjMembers::getByWpUserId($userId);
        $member = $memberObj ? $memberObj->toArray() : null;
        if (!$member || !in_array($member['role'], [MjRoles::COORDINATEUR], true)) {
            wp_send_json_error(['message' => __('Accès refusé.', 'mj-member')], 403);
        }
        ...
        $result = MjLeaveRequests::approve($requestId, (int) $member['id'], $comment);
        if (is_wp_error($result)) {
            wp_send_json_error(['message' => $result->get_error_message()], 500);
        }

        wp_send_json_success(['message' => __('Demande approuvée.', 'mj-member')]);
    }

    /**
     * Reject a leave request (coordinator only).
     */
    public function reject(): void
    {
        ...
        if (empty($comment)) {
            wp_send_json_error(['message' => __('Un motif de refus est requis.', 'mj-member')], 400);
        }
        ...
        $result = MjLeaveRequests::reject($requestId, (int) $member['id'], $comment);
        if (is_wp_error($result)) {
            wp_send_json_error(['message' => $result->get_error_message()], 500);
        }

        wp_send_json_success(['message' => __('Demande refusée.', 'mj-member')]);
    }

    /**
     * Get leave data for a specific member (coordinator view).
     */
    public function byMember(): void
    {
        ...
        $currentMemberObj = MjMembers::getByWpUserId($userId);
        $currentMember = $currentMemberObj ? $currentMemberObj->toArray() : null;
        if (!$currentMember || !in_array($currentMember['role'], [MjRoles::COORDINATEUR], true)) {
            wp_send_json_error(['message' => __('Accès refusé.', 'mj-member')], 403);
        }
        ...
        wp_send_json_success([
            'member'   => [
                'id'   => (int) $memberArr['id'],
                'name' => trim($memberArr['first_name'] . ' ' . $memberArr['last_name']),
                'role' => $memberArr['role'],
            ],
            'quotas'   => $quotas,
            'usage'    => $usage,
    ...
    /**
     * Create a leave request on behalf of a member (coordinator only).
     */
    public function createByCoordinator(): void
    {
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'mj-leave-requests')) {
            wp_send_json_error(['message' => __('Sécurité échouée.', 'mj-member')], 403);
        }
        ...
        $requestId = MjLeaveRequests::create([
            'member_id'        => $memberId,
            'type_id'          => $typeId,
            'status'           => MjLeaveRequests::STATUS_APPROVED,
            'dates'            => wp_json_encode($validDates),
            'reason'           => $reason,
            'reviewed_by'      => (int) $currentMember['id'],
            'reviewed_at'      => current_time('mysql'),
            'reviewer_comment' => __('Créé par coordinateur', 'mj-member'),
        ]);
        ...
        wp_send_json_success(['message' => __('Demande créée avec succès.', 'mj-member')]);
    }

    /**
     * Delete a leave request (coordinator only).
     */
    public function deleteRequest(): void
    {
        ...
        $requestId = isset($_POST['request_id']) ? (int) $_POST['request_id'] : 0;
        if ($requestId <= 0) {
            wp_send_json_error(['message' => __('Demande invalide.', 'mj-member')], 400);
        }
        ...
        $result = MjLeaveRequests::delete($requestId);
        if (is_wp_error($result)) {
            wp_send_json_error(['message' => $result->get_error_message()], 500);
        }

        wp_send_json_success(['message' => __('Demande supprimée.', 'mj-member')]);
    }

    private function notifyCreated(int $requestId, array $member, $type, array $dates): void
    {
        ...
        mj_member_record_notification(
            [
                ...
                'url' => home_url('/mon-compte/ressources-humaine/?member_id=' . (int) $member['id']),
                'payload' => [
...
/**
 * Localize leave requests script data.
 * Global function preserved for backward compatibility (called from AssetsManager).
 *
 * @return void
 */
function mj_member_leave_requests_localize(): void
{
    LeaveRequestsController::localize();
}