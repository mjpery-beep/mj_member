...
final class PaymentsController implements AjaxHandlerInterface
{
    public function registerHooks(): void
    {
        add_action('wp_ajax_mj_admin_get_qr', [$this, 'adminGetQr']);
        add_action('wp_ajax_mj_admin_get_payment_history', [$this, 'adminGetPaymentHistory']);
        add_action('wp_ajax_mj_admin_delete_payment', [$this, 'adminDeletePayment']);
        add_action('wp_ajax_mj_member_mark_paid', [$this, 'markPaid']);
        add_action('wp_ajax_mj_admin_generate_event_payment_link', [$this, 'adminGenerateEventPaymentLink']);
    }

    public function adminGenerateEventPaymentLink(): void
    {
    ...
    public function adminGetQr(): void
    {
    ...
    public function adminGetPaymentHistory(): void
    {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'mj_admin_payments_nonce')) {
        wp_send_json_error('Nonce invalide.');
    }
    ...
    if ($history_rows) {
        foreach ($history_rows as $row) {
            ...
            $entries[] = array(
                ...
                'reference' => $reference,
                'status' => $status_key,
                'status_label' => $this->formatPaymentStatus($status_key),
                'method' => $method_label,
                'context' => $context_key,
                'context_label' => $this->formatPaymentContextLabel($context_key, $event_title),
    ...
    private function formatPaymentContextLabel(string $context, string $event_title = ''): string
    {
    ...
    public function adminDeletePayment(): void
    {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'mj_admin_payments_nonce')) {
        wp_send_json_error('Nonce invalide.');
    }
