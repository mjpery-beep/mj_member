...
final class FixturesMediaImportController implements AjaxHandlerInterface
{
    public function registerHooks(): void
    {
        ...
        $isFixturesAction = in_array($action, array(
            'mj_member_fixture_media_restore_start',
            'mj_member_fixture_media_restore_progress',
            'mj_member_fixture_media_restore_worker',
        ), true);

        $page = isset($_REQUEST['page']) ? sanitize_key((string) wp_unslash($_REQUEST['page'])) : '';
        if (!$isFixturesAction && $page !== 'mj_settings') {
            return;
        }
