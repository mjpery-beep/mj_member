...
final class LeaveRequestsController implements AjaxHandlerInterface
{
    ...
    {
        add_action('wp_ajax_mj_leave_request_approve', [$this, 'memberLeaveRequestApprove']);
        add_action('wp_ajax_mj_leave_request_reject', [$this, 'memberLeaveRequestReject']);
        add_action('wp_ajax_mj_leave_request_list', [$this, 'memberLeaveRequestList']);
        add_action('wp_ajax_mj_leave_request_by_member', [$this, 'memberLeaveRequestByMember']);
        add_action('wp_ajax_mj_leave_request_create_by_coordinator', [$this, 'memberLeaveRequestCreateByCoordinator']);
        add_action('wp_ajax_mj_leave_request_delete', [$this, 'memberLeaveRequestDelete']);
    }
     ...
     */
    private function memberLeaveRequestsCheckCoordinator()
    {
     ...
     * @return void
     */
    public function memberLeaveRequestApprove(): void
    {
        // Verify nonce
        ...
        $coordinator = $this->memberLeaveRequestsCheckCoordinator();
        if (!$coordinator) {
        ...
        // Notify the member
        $this->memberNotifyLeaveRequestReviewed($requestId, 'approved');
     ...
     * @return void
     */
    public function memberLeaveRequestReject(): void
    {
        // Verify nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'mj-leave-requests')) {
            wp_send_json_error(['message' => __('Sécurité échouée.', 'mj-member')], 403);
        ...
        // Notify the member
        $this->memberNotifyLeaveRequestReviewed($requestId, 'rejected');
     ...
     * @return void
     */
    public function memberLeaveRequestList(): void
    {
        // Verify nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'mj-leave-requests')) {
            wp_send_json_error(['message' => __('Sécurité échouée.', 'mj-member')], 403);
     ...
     * @return void
     */
    public function memberLeaveRequestByMember(): void
    {
        // Verify nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'mj-leave-requests')) {
            wp_send_json_error(['message' => __('Sécurité échouée.', 'mj-member')], 403);
     ...
     * @return void
     */
    private function memberNotifyLeaveRequestReviewed(int $requestId, string $action): void
    {
        ...
        mj_member_record_notification(
            [
                ...
                'title' => $title,
                'excerpt' => $excerpt,
                'url' => home_url('/mon-compte/ressources-humaine/'),
                'payload' => [
     ...
     * @return void
     */
    public function memberLeaveRequestCreateByCoordinator(): void
    {
        // Verify nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'mj-leave-requests')) {
            wp_send_json_error(['message' => __('Sécurité échouée.', 'mj-member')], 403);
     ...
     * @return void
     */
    public function memberLeaveRequestDelete(): void
    {
        // Verify nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'mj-leave-requests')) {
            wp_send_json_error(['message' => __('Sécurité échouée.', 'mj-member')], 403);
