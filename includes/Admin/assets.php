...
function mj_custom_admin_styles($hook) {
    $plugin_root_file = MJ_MEMBER_PATH . 'mj-member.php';

    wp_enqueue_style('custom-styles', plugins_url('css/styles.css', $plugin_root_file));
    ...
    wp_enqueue_script('mj-inline-edit', plugins_url('js/inline-edit.js', $plugin_root_file), array('jquery'), $inline_edit_version, true);
    wp_enqueue_media();
    wp_enqueue_script('mj-photo-upload', plugins_url('js/photo-upload.js', $plugin_root_file), array('jquery'), '1.0.0', true);
    ...
    wp_enqueue_script('mj-admin-payments', plugins_url('includes/js/admin-payments.js', $plugin_root_file), array('jquery'), $payments_js_version, true);
    ...
    wp_enqueue_script('mj-member-user-link', plugins_url('js/member-user-link.js', $plugin_root_file), array('jquery'), $user_link_js_version, true);
    ...
    if (is_string($hook) && strpos($hook, 'mj_send_emails') !== false) {
        ...
        wp_enqueue_style('mj-member-admin-send-emails', plugins_url('css/admin-send-emails.css', $plugin_root_file), array(), $send_emails_css_version);
        ...
        wp_enqueue_script('mj-member-admin-send-emails', plugins_url('js/admin-send-emails.js', $plugin_root_file), array('jquery'), $send_emails_js_version, true);
