...
class MjEventAnimateurs {
    ...
    public static function get_event_ids_for_member($animateur_id) {
        $animateur_id = (int) $animateur_id;
        if ($animateur_id <= 0) {
            return array();
        }
        ...
        if (self::is_ready()) {
            ...
            $table = self::get_table_name();
            $results = $wpdb->get_col($wpdb->prepare(
                "SELECT event_id FROM {$table} WHERE animateur_id = %d ORDER BY event_id ASC",
                $animateur_id
            ));
        ...
        if (function_exists('mj_member_column_exists')) {
            ...
            if (mj_member_table_exists($events_table) && mj_member_column_exists($events_table, 'animateur_id')) {
                $legacy_ids = $wpdb->get_col($wpdb->prepare(
                    "SELECT id FROM {$events_table} WHERE animateur_id = %d",
                    $animateur_id
                ));
                ...
                if ($wp_user_id > 0) {
                    $legacy_user_ids = $wpdb->get_col($wpdb->prepare(
                        "SELECT id FROM {$events_table} WHERE animateur_id = %d",
                        $wp_user_id
                    ));
                    if (is_array($legacy_user_ids)) {
                        foreach ($legacy_user_ids as $value) {
                            $value = (int) $value;
                            if ($value > 0) {
                                $event_ids[$value] = $value;
                            }
    ...
    public static function get_events_for_member($animateur_id, array $args = array()) {
        $animateur_id = (int) $animateur_id;
        if ($animateur_id <= 0) {
            return array();
        }

        $event_ids = self::get_event_ids_for_member($animateur_id);
        if (empty($event_ids)) {
            return array();
        }
    ...
    public static function member_is_assigned($event_id, $member_id) {
        ...
        if ($event && isset($event->animateur_id) && $wp_user_id > 0 && (int) $event->animateur_id === $wp_user_id) {
            return true;
        }
