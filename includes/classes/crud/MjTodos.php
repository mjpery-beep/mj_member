...
class MjTodos extends MjTools implements CrudRepositoryInterface
{
    ...
    public static function create($data)
    {
        ...
        $emoji = self::sanitize_emoji_value($data['emoji'] ?? '');
    ...
    public static function sanitize_emoji_value($value): string
    {
        ...
        $sanitized = wp_strip_all_tags((string) $value, false);
        ...
        if (function_exists('grapheme_substr')) {
            $sanitized = grapheme_substr($sanitized, 0, 8);
        } elseif (function_exists('mb_substr')) {
            $sanitized = mb_substr($sanitized, 0, 16, 'UTF-8');
        } else {
            $sanitized = substr($sanitized, 0, 16);
        }
