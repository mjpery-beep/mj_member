...
class MjEventRegistrations implements CrudRepositoryInterface {
    ...
    private static function validate_age($event, $member) {
        ...
        $reference = self::resolve_age_reference_timestamp($event);
     ...
     * @return int
     */
    private static function resolve_age_reference_timestamp($event) {
        ...
        $raw_start = trim((string) $event->date_debut);
        if ($raw_start === '' || strpos($raw_start, '0000-00-00') === 0) {
            return (int) $reference;
        }

        $start_timestamp = strtotime($raw_start);
