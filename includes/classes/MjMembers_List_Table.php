...
class MjMembers_List_Table extends WP_List_Table {
    ...
    public function column_detail($item) {
        ...
        if (!empty($item->requires_payment)) {
            $suffix = ($item->role === MjMembers_CRUD::ROLE_TUTEUR) ? ' ' . esc_html__('(paye pour ses jeunes)', 'mj-member') : '';
            $cotisation_display = '<span class="mj-detail-badge mj-detail-badge--required">' . esc_html__('Obligatoire', 'mj-member') . $suffix . '</span>';
        } else {
            $cotisation_display = '<span class="mj-detail-badge mj-detail-badge--exempt">' . esc_html__('Dispensé', 'mj-member') . '</span>';
        }
        $finance_rows[] = $this->renderDetailRow('🎟️', __('Cotisation', 'mj-member'), $cotisation_display);

        $payment_meta = $this->getPaymentStatusMeta($item);
        $payment_modifier = sanitize_html_class($payment_meta['modifier']);
        $payment_parts = array();
        $payment_parts[] = '<span class="mj-payment-status-pill mj-payment-status-pill--' . $payment_modifier . '">' . esc_html($payment_meta['label']) . '</span>';

        if ($payment_meta['requires']) {
            $payment_parts[] = '<div class="mj-detail-meta">' . $this->renderLastPaymentValue($item) . '</div>';
        } else {
            $payment_parts[] = '<div class="mj-detail-muted">' . esc_html__('Cotisation non requise', 'mj-member') . '</div>';
        }
        $finance_rows[] = $this->renderDetailRow('💶', __('Paiement', 'mj-member'), '<div class="mj-detail-payment">' . implode('', $payment_parts) . '</div>');

        $finance_rows[] = $this->renderDetailRow('📸', __('Consentement photo', 'mj-member'), $this->buildDetailPhotoConsent($item));
        ...
        if (!empty($item->date_inscription)) {
            $timestamp = strtotime((string) $item->date_inscription);
            if ($timestamp) {
                $registration_text = wp_date('d/m/Y', $timestamp);
            }
        }

        $registration_html = $registration_text !== ''
            ? '<span class="mj-detail-meta">' . esc_html($registration_text) . '</span>'
            : '<span class="mj-detail-muted">' . esc_html__('Date inconnue', 'mj-member') . '</span>';

        $sections[] = $this->renderDetailGrid(array(
            $this->renderDetailRow('🗓️', __('Inscription', 'mj-member'), $registration_html),
        ));
    ...
    private function renderDetailRow($icon, $label, $valueHtml) {
        ...
        return '<div class="mj-detail-row" data-detail-label="' . esc_attr($label) . '"' . $title_attr . '>' . $icon_html . '<div class="mj-detail-value">' . $valueHtml . '</div></div>';
    }
