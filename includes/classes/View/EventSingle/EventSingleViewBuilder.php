...

{
...

    {
        ...
        $this->state['poster_image_url'] = $this->resolveString('poster_image_url', $event['poster_image_url'] ?? '');
    ...
    private function prepareScheduleComponent(): void

    {
        ...
        $schedule = $this->buildScheduleContext($event);
        ...
        if (class_exists(ScheduleDisplayHelper::class)) {
            ...
            $component = ScheduleDisplayHelper::render($schedule, array(
        ...
        $this->state['schedule_component'] = $component;
     ...
     */
    private function buildScheduleContext(array $event): array

    {

        $mode = '';

        if (isset($event['schedule_mode']) && !is_array($event['schedule_mode'])) {

            $mode = (string) $event['schedule_mode'];
        ...
        if (isset($this->state['occurrence_schedule_summary']) && !is_array($this->state['occurrence_schedule_summary'])) {

            $scheduleSummary = (string) $this->state['occurrence_schedule_summary'];

        } elseif (isset($event['occurrence_schedule_summary']) && !is_array($event['occurrence_schedule_summary'])) {

            $scheduleSummary = (string) $event['occurrence_schedule_summary'];

        } elseif (isset($event['schedule_summary']) && !is_array($event['schedule_summary'])) {

            $scheduleSummary = (string) $event['schedule_summary'];
        ...
        if (isset($this->state['display_date_label']) && !is_array($this->state['display_date_label'])) {

            $displayLabel = (string) $this->state['display_date_label'];

        } elseif (isset($event['display_date_label']) && !is_array($event['display_date_label'])) {

            $displayLabel = (string) $event['display_date_label'];

        }



        $dateDebut = isset($event['date_debut']) && !is_array($event['date_debut'])

            ? (string) $event['date_debut']

            : '';



        $dateFin = isset($event['date_fin']) && !is_array($event['date_fin'])

            ? (string) $event['date_fin']

            : '';



        $occurrences = $this->buildScheduleOccurrences();

        $nextOccurrence = $this->extractNextOccurrenceEntry($occurrences);

        $nextOccurrenceLabel = $this->resolveNextOccurrenceLabel($nextOccurrence);
        ...
        $schedule = array(
            ...
            'weekly_schedule' => $this->buildWeeklyScheduleData($event),
        ...
        if ($nextOccurrenceLabel !== '') {

            $schedule['next_occurrence_label'] = $nextOccurrenceLabel;
     ...
     */
    private function buildScheduleOccurrences(): array

    {

        $source = array();

        if (!empty($this->state['occurrence_preview']) && is_array($this->state['occurrence_preview'])) {

            $preview = $this->state['occurrence_preview'];
        ...
        if (empty($source) && isset($this->state['occurrence_items']) && is_array($this->state['occurrence_items'])) {

            $source = $this->state['occurrence_items'];
     ...
     * @return array<string,mixed>
     */
    private function extractNextOccurrenceEntry(array $occurrences): array

    {
        ...
        if (!empty($this->state['occurrence_preview']) && is_array($this->state['occurrence_preview'])) {
            ...
            if (isset($preview['next']) && is_array($preview['next'])) {
                ...
                if (!empty($next['label'])) {
                    ...
                    $result['label'] = sanitize_text_field((string) $next['label']);
                ...
                if (!empty($next['start'])) {
                    ...
                    $result['start'] = (string) $next['start'];

                }



                if (!empty($next['time_start'])) {

                    $result['time_start'] = (string) $next['time_start'];

                }



                if (!empty($next['time_end'])) {

                    $result['time_end'] = (string) $next['time_end'];
                ...
                if (!empty($next['timestamp'])) {
                    ...
                    $result['timestamp'] = (int) $next['timestamp'];
    ...
    /**
     * @param array<string,mixed> $nextOccurrence
     */
    private function resolveNextOccurrenceLabel(array $nextOccurrence): string

    {

        if (isset($this->state['occurrence_next_label']) && !is_array($this->state['occurrence_next_label'])) {

            $candidate = (string) $this->state['occurrence_next_label'];
        ...
        if (!empty($nextOccurrence['label'])) {

            return sanitize_text_field((string) $nextOccurrence['label']);
        ...
        $timestamp = strtotime((string) $nextOccurrence['start']);
        ...
        $dateLabel = wp_date(get_option('date_format', 'd/m/Y'), $timestamp);

        if ($dateLabel === '') {

            $dateLabel = date_i18n('d/m/Y', $timestamp);

        }



        $timeStart = isset($nextOccurrence['time_start']) ? $this->formatTimeForDisplay((string) $nextOccurrence['time_start']) : '';

        $timeEnd = isset($nextOccurrence['time_end']) ? $this->formatTimeForDisplay((string) $nextOccurrence['time_end']) : '';
        ...
        if ($timeStart !== '') {
            ...
            if ($timeEnd !== '' && $timeEnd !== $timeStart) {
                ...
                return sprintf('%s · %s → %s', $dateLabel, $timeStart, $timeEnd);

            }



            return sprintf('%s · %s', $dateLabel, $timeStart);
     ...
     * @return array<string,mixed>
     */
    private function buildWeeklyScheduleData(array $event): array

    {
        ...
        $payload = $this->decodeSchedulePayload($event['schedule_payload'] ?? array());

        $showDateRange = !empty($payload['show_date_range']);



        if ($mode === 'series') {

            return $this->buildSeriesScheduleData($payload, $showDateRange);
        ...
        if ($frequency === 'monthly') {
            ...
            return $this->buildMonthlyScheduleData($payload, $showDateRange);
        ...
        if ($frequency === 'weekly') {
            ...
            return $this->buildRecurringWeeklySchedule($payload, $showDateRange);
     ...
     * @return array<string,mixed>
     */
    private function decodeSchedulePayload($payload): array
     ...
     * @return array<string,mixed>
     */
    private function buildRecurringWeeklySchedule(array $payload, bool $showDateRange): array

    {
        ...
        foreach ($weekdays as $dayKey) {
            ...
            if (isset($weekdayTimes[$dayKey]) && is_array($weekdayTimes[$dayKey])) {

                $dayTimes = $weekdayTimes[$dayKey];

                if (isset($dayTimes['start']) && $dayTimes['start'] !== '') {

                    $startTime = (string) $dayTimes['start'];

                }



                if (isset($dayTimes['end']) && $dayTimes['end'] !== '') {

                    $endTime = (string) $dayTimes['end'];
     ...
     * @return array<string,mixed>
     */
    private function buildMonthlyScheduleData(array $payload, bool $showDateRange): array

    {
        ...
        $monthlyLabel = trim(sprintf('%s %s %s', $ordinalLabel, $weekdayLabel, __('du mois', 'mj-member')));
     ...
     * @return array<string,mixed>
     */
    private function buildSeriesScheduleData(array $payload, bool $showDateRange): array

    {
        ...
        if (!empty($seriesItems)) {
            ...
            usort($seriesItems, static function ($left, $right): int {

                $leftTs = isset($left['timestamp']) ? (int) $left['timestamp'] : 0;

                $rightTs = isset($right['timestamp']) ? (int) $right['timestamp'] : 0;
